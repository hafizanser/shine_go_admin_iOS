...
extension ImagePickerCustomViewController {
    ...
    @IBAction func doneButtonWasPressed(_ sender: UIButton) {
        if selectedIndex == -1 {
            self.alertMessage(title: K.ALERT, alertMessage: "Please select at least one item", action: nil)
        } else {
            navigationController?.popViewController(animated: true)
            let selectedPhotos = [allPhotos[selectedIndex]]
